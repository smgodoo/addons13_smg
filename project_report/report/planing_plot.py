...
class ProjectForecastXlsx(models.AbstractModel):
    _name = 'report.project_report.project_forecast_xlsx'
    ...
    def generate_forecast_xlsx_report(self, workbook, data, partners):
        ...
        sheet.write(0, 0, '[DEPARTMENT NAME {} BUDGET + TIMINGS'.format(today.year))
        ...
        print(total_column, from_column_letter, last_column_letter)
        ...
        if self.env.user.has_group('project_owner.project_owner_group_project_see_all_vt'):
            departments = self.env['hr.department'].search([])
            for department in departments:
                sheet.set_row(start_record_row, 22)
                sheet.write(start_record_row, start_record_column, department.name, department_cell_format)
                start_record_row += 1
                ...
                if project_forecasts:
                    for forecast in project_forecasts:
                        ...
                        print(start_week, end_week, date_start, end_date, forecast_date_start, forecast_date_end)
                        sheet.set_row(start_record_row, 22)
                        ...
                        if forecast.project_id.project_budget:
                            sheet.write(start_record_row, start_record_column + 4, forecast.project_id.project_budget,
                                        normal_cell_format)
            ...
            if employee:
                departments = self.env['hr.department'].search([('parent_id', '=', employee.department_id.id)])
                print(departments)
                for department in departments:
                    sheet.set_row(start_record_row, 22)
                    sheet.write(start_record_row, start_record_column, department.display_name, department_cell_format)
                    start_record_row += 1
                    project_forecasts = self.env['planning.slot'].search(
                        [('employee_id.department_id', '=', department.id)])
                    if project_forecasts:
                        for forecast in project_forecasts:
                            date_start = fields.Date.from_string(forecast.start_date)
                            end_date = fields.Date.from_string(forecast.end_datetime)
                            start_week = self.find_day_in_week(days_in_month, date_start)
                            end_week = self.find_day_in_week(days_in_month, end_date)
                            forecast_date_start = self.find_column_write_dateline(start_week, date_start)
